...
3. 运行健康层（`health`）
- 提供库级 health probe 接口与标准结果模型（`status/code/message/metadata/checked_at`）。
- 支持 `probe_timeout` 与 `cache_ttl` 语义，输出 deterministic canonical reason codes。
...
- allowlist 激活默认值（由 `runtime/config` 生效）：
  ...
  - `adapter.allowlist.on_unknown_signature=deny`
- manifest allowlist identity 最小字段：`allowlist.adapter_id`、`allowlist.publisher`、`allowlist.version`、`allowlist.signature_status`。
- adapter runtime health 默认值（由 `runtime/config` 生效）：
