...
- 新增“真实逻辑落地”门禁与 README 同步门禁（A62 后续增量）：
  - `check-agent-mode-real-logic-contract.sh/.ps1`：
    - 阻断 `examples/agent-modes/*/*/main.go` 引用 `examples/agent-modes/internal/agentmode`；
    - 阻断仅输出元数据/占位文本、未触发主干真实执行路径的示例；
    - 输出可审计失败分类：`agent-mode-simulated-engine-dependency`、`agent-mode-placeholder-output-regression`、`agent-mode-missing-mainline-runtime-path`。
  - `check-agent-mode-readme-sync-contract.sh/.ps1`：
    - 当 `examples/agent-modes/*/*/main.go` 发生行为变更时，强制对应 README 同步更新；
    - README 至少覆盖 `Run`、`Prerequisites`、`Real Runtime Path`、`Expected Output/Verification`；
    - 输出可审计失败分类：`agent-mode-readme-not-updated`、`agent-mode-readme-missing-required-sections`。
- 新增“示例稳定性/性能治理”条件触发约束（并入 a62，不新开平行提案）：
  - 当 `agent-mode` 示例回归出现时延超预算或波动（flaky）超阈值时，必须在 a62 内增量补齐治理；
  - 治理范围至少包括：分片执行策略、耗时预算、flaky 分类与重试策略、阻断门禁接线。
- 新增“提案级示例影响评估”约束：
...
- 质量门禁脚本：`scripts/check-agent-mode-examples-smoke.*`、`scripts/check-agent-mode-pattern-coverage.*`、`scripts/check-agent-mode-migration-playbook-consistency.*`、`scripts/check-agent-mode-legacy-todo-cleanup.*`、`scripts/check-agent-mode-real-logic-contract.*`、`scripts/check-agent-mode-readme-sync-contract.*`、`scripts/check-quality-gate.*`。
