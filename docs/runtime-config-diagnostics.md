...
- 示例：
  ...
  - `runtime.memory.mode` -> `BAYMAX_RUNTIME_MEMORY_MODE`
  - `runtime.memory.external.provider` -> `BAYMAX_RUNTIME_MEMORY_EXTERNAL_PROVIDER`
  - `runtime.memory.external.profile` -> `BAYMAX_RUNTIME_MEMORY_EXTERNAL_PROFILE`
  - `runtime.memory.external.contract_version` -> `BAYMAX_RUNTIME_MEMORY_EXTERNAL_CONTRACT_VERSION`
  - `runtime.memory.builtin.root_dir` -> `BAYMAX_RUNTIME_MEMORY_BUILTIN_ROOT_DIR`
  - `runtime.memory.builtin.compaction.enabled` -> `BAYMAX_RUNTIME_MEMORY_BUILTIN_COMPACTION_ENABLED`
...
- Context Compression Runtime Handoff：handoff 采用 `handoff.v1` 有界结构，仅记录结构化事实、推断、待确认事项与 Artifact/Checkpoint/Session History/Snapshot 引用；质量不足、非法切点和引用丢失使用 `handoff_*` additive 诊断字段表示，恢复不复制既有事实源。handoff 同时投影已有 Context Assembler 的 pressure zone/reason、compaction fallback/quality、保留证据计数、spill/swap-back、lifecycle tier、cold-store governance 与 recovery consistency 元数据，不携带对应正文；恢复以稳定 handoff ID 去重，且在首次 mutation 前完成 schema/reference 校验。`context.handoff` 是仅含投影的生命周期事件，`context_handoff_event` 取 `generated|fallback|generation_failed|validated|restored`；其余 `context_handoff_version`、`context_handoff_cut`、`context_handoff_quality_score`、`context_handoff_fallback_reason`、`context_handoff_restore_ready` 会由 `RuntimeRecorder` 合并到相同 Run 的最终 `run.finished` 诊断记录，缺失时保持 nullable/default 兼容。
- handoff 配置位于 `runtime.context.jit.compaction.handoff.*`：`enabled` 默认关闭，`max_serialized_bytes`、`max_latency_ms`、`quality_threshold` 和 `failure_policy` 均在 `env > file > default` 下解析；非法值 fail-fast，热更新失败原子回滚。
...
- extension lifecycle fields are additive and nullable: `extension_name`、`extension_generation`、`extension_phase`、`extension_reason`、`extension_source`、`extension_digest`；unknown or missing values preserve default compatibility and lifecycle events are parsed only through `RuntimeRecorder`.
- external extension authoring conformance 使用 `external_extension_authoring.v1` 离线 fixture；不新增 runtime 配置键。结果字段仅包含 bounded `status`、`phase`、`reason_code`、`field`、`expected`、`actual`、`remediation`，不得写入源码、凭据、reasoning、workspace 或无界 payload。
...
- Embedded host correlation introduces no runtime configuration keys. Host admission, pending-close, delivery, correlation, and source-control observations are bounded standard events projected by `observability/event.RuntimeRecorder`; cursor bodies, raw frames, and arbitrary payloads are excluded. A recorded host delivery drop uses the existing durable Realtime `drop_with_record` source policy only for source-declared `delta` projections; no EventSink, direct Runner callbacks, and non-delta events remain non-droppable. Host disconnect does not mutate source Run state.
...
- drift 分类（Cross-Domain Primary Reason Arbitration/Arbitration Explainability + Secondary Reason/Arbitration Version Governance blocking）：
  ...
  - `secondary_order_drift`：secondary reasons 顺序或去重语义漂移。
  - `secondary_count_drift`：secondary count 或上限语义漂移。
  - `hint_taxonomy_drift`：remediation hint taxonomy 漂移。
  - `rule_version_drift`：arbitration rule version 漂移。
