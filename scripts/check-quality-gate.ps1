...
Invoke-RequiredParallelSteps -Steps @(
    ...
    @{
        StepLabel = "[quality-gate] action capability risk/idempotency evidence audit contract"
    ...
    @{
        StepLabel = "[quality-gate] embedded host contract suites"
        Command   = { pwsh -File scripts/check-embedded-host-contract.ps1 }
    },
    @{
        StepLabel = "[quality-gate] runtime steering/follow-up contract suites"
        Command   = { pwsh -File scripts/check-runtime-steering-follow-up-contract.ps1 }
    },
    @{
    ...
    @{
        StepLabel = "[quality-gate] event stream terminal recovery contract"
