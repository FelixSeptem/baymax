...
Write-Host "[agent-mode-readme-sync-contract] validating README sync for agent-mode examples"
...
if ($mainFiles.Count -eq 0) {
    throw "[agent-mode-readme-sync-contract] no agent-mode main.go found"
}
...
foreach ($mainPath in $changedMainFiles) {
    ...
    foreach ($changedReadme in $changedReadmeFiles) {
        $normalized = $changedReadme.Replace("\", "/")
        if ($normalized -eq $readmePath) {
            $matched = $true
            break
        }
    }
...
if ($readmeNotUpdated.Count -gt 0) {
    Write-Host "[agent-mode-readme-sync-contract][agent-mode-readme-not-updated] main.go changed without matching README update:"
...
if ($missingRequiredSections.Count -gt 0) {
    Write-Host "[agent-mode-readme-sync-contract][agent-mode-readme-missing-required-sections] required README sections missing:"
...
Write-Host "[agent-mode-readme-sync-contract] README sync contract passed"
