...
| 条件候选 | 跨 Provider handoff 与 stream edge conformance | cross-provider、abort usage、overflow、Unicode fixtures | 优先扩展测试/fixture/gate，复用现有 provider admission 和 error taxonomy | 生产出现跨 provider 恢复或边界兼容回归。 |
