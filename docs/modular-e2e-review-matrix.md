...
| `context/*` | 仅策略编排，不直接依赖 provider SDK | stage fail-fast/best-effort 行为一致 | context pressure 状态更新与 spill/swap 单进程一致 | assemble 诊断字段完整 |
...
| `Context Stage2` | rules 路由与 stage policy 行为可预测 | `core/runner/runner_test.go::TestRunContextStage2BestEffortKeepsModelPath`, `TestStreamContextStage2FailFastStopsBeforeModel` |
| `Context pressure/recovery` | zone 与动作语义 Run/Stream 一致 | `core/runner/runner_test.go::TestRunAndStreamContextPressureSemanticsEquivalent`, `context/assembler/assembler_test.go::TestAssemblerCA3SpillIdempotentAcrossRetry` |
...
1. Context pressure `TokenCounter` 在 `Run/Stream` 的 selected-provider 注入路径缺少契约测试。
