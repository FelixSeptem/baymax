...
The runtime SHALL expose checkpoint relation, parent, branch, history index, restore source, and replay identity as additive optional references. The projection MUST validate root/derived/branch/replay relationships, ordered history continuity, schema compatibility, and replay idempotency without mutating snapshot state or creating a history store.

#### Scenario: Root checkpoint is projected without a parent
- **WHEN** a valid snapshot manifest is projected as a root checkpoint
- **THEN** the result retains manifest schema/source/digest and omits parent and branch references

#### Scenario: Derived checkpoint requires an existing parent
- **WHEN** a derived, branch, or replay checkpoint omits its parent or references a missing history entry
...
- **WHEN** the same replay key is projected with different normalized checkpoint data
