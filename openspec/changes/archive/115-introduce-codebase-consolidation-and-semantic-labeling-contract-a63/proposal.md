...
- 运行时 Harness 架构文档收口：在文档治理范围内补齐单一总览入口（建议 `docs/runtime-harness-architecture.md`），统一描述 `state surfaces -> guides/sensors -> tool mediation -> entropy control` 与主干 contract/gate 映射，避免架构语义散落在多文档重复维护。
