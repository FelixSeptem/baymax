...
if rg -n --glob 'host/**/*.go' --glob 'host/*.go' --glob '!host/**/*_test.go' --glob '!host/*_test.go' '(steering|follow[-_ ]?up|hosted listener|remote Session|Artifact store|global queue|terminal state machine)' .; then
