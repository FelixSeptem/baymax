...
echo "[agent-mode-readme-sync-contract] validating README sync for agent-mode examples"

mapfile -t main_files < <(find examples/agent-modes -mindepth 3 -maxdepth 3 -type f -name main.go | sort)
if (( ${#main_files[@]} == 0 )); then
  echo "[agent-mode-readme-sync-contract] no agent-mode main.go found" >&2
  exit 1
fi
...
if (( ${#readme_not_updated[@]} > 0 )); then
  echo "[agent-mode-readme-sync-contract][agent-mode-readme-not-updated] main.go changed without matching README update:" >&2
  printf '  - %s\n' "${readme_not_updated[@]}" >&2
fi

if (( ${#missing_required_sections[@]} > 0 )); then
  echo "[agent-mode-readme-sync-contract][agent-mode-readme-missing-required-sections] required README sections missing:" >&2
...
echo "[agent-mode-readme-sync-contract] README sync contract passed"
