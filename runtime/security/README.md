...
- JSON 文本脱敏
- 关键字匹配与可扩展 matcher
- A57 sandbox egress / adapter allowlist 治理语义的安全域约束说明（配置与执行实现在 `runtime/config`、`core/runner`、`adapter/manifest`）
- A58 policy precedence / decision trace 治理边界说明（precedence evaluator 在 `runtime/config`，执行透传在 `core/runner`）
...
- A57 治理边界遵循同一原则：`runtime/security` 不直接承载 egress 判定与 allowlist 激活状态机，仅维护安全能力边界说明；实际判定由上层合同模块落地。
- A58 治理边界保持一致：`runtime/security` 不承载 precedence 评估器或 winner 计算，不直接写入 `policy_decision_path`；仅维持安全域能力边界，决策链路由 `runtime/config` + `core/runner` + `observability/event` 统一实现。

## A57 治理边界说明

A57 引入的 `security.sandbox.egress.*` 与 `adapter.allowlist.*` 配置域属于“安全治理合同”，但实现分层保持不变：
...
- policy precedence replay 与 gate（A58）：`tool/diagnosticsreplay` + `scripts/check-policy-precedence-contract.*`
...
- 安全治理联调（A57）：`pwsh -File scripts/check-sandbox-egress-allowlist-contract.ps1`（或 shell 同名脚本）验证 egress/allowlist 合同阻断语义。
- precedence 治理联调（A58）：`pwsh -File scripts/check-policy-precedence-contract.ps1`（或 shell 同名脚本）验证跨层 winner 与 decision trace 合同稳定性。
