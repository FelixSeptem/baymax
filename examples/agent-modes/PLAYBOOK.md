...
| `realtime-interrupt-resume` | `P2` | `embedded_host.command_response_event_correlation+runtime_input.steering_follow_up_safe_point+provider_handoff.canonical_stream_edge_projection` | `governance_realtime_gate_enforced; governance_realtime_replay_bound; runtime_steering_admitted; runtime_steering_applied_at_safe_point; runtime_follow_up_pending; runtime_follow_up_promoted; runtime_input_not_applied_on_disconnect; realtime_stream_binding_expired; realtime_stream_binding_backpressure; realtime_stream_binding_disconnect_recovery; provider_tool_call_normalized; provider_feedback_correlated; provider_stream_boundary_normalized; provider_abort_usage_nullable; provider_fallback_fence_enforced; provider_run_stream_parity` | `check-realtime-protocol-contract.*; check-agent-runtime-protocol-contract.*; check-runtime-event-stream-terminal-recovery-contract.*; check-runtime-steering-follow-up-contract.*; check-embedded-host-contract.*; check-cross-provider-handoff-stream-edge-conformance.*` | `embedded_host_protocol.v1; realtime_event_protocol.v1; agent_runtime_protocol.v1/stream-binding.json; runtime_event_stream_terminal_recovery.v1; provider_handoff_stream_edge.v1` |
| `agent-runtime-protocol-projection` | `P1` | `agent_runtime_protocol.capability_context_admission_terminal_outcome` | `governance_agent_runtime_protocol_gate_enforced; governance_agent_runtime_protocol_replay_bound; terminal_outcome_projected` | `check-agent-runtime-protocol-contract.*; check-terminal-outcome-contract.*` | `agent_runtime_protocol.v1; terminal_outcomes.v1` |
| `checkpoint-workspace-provenance` | `P3` | `agent_runtime_protocol.checkpoint_history_workspace_provenance` | `checkpoint_replay_idempotent; workspace_integrity_drift_classified; governance_checkpoint_provenance_replay_bound` | `check-agent-runtime-protocol-contract.*; check-state-snapshot-contract.*` | `agent_runtime_protocol_checkpoint_provenance.v1` |
| `session-history-checkpoint-replay` | `P2` | `session.history_leaf_branch_restore_replay` | `restore_conflict_classified; run_stream_history_replay_parity; replay_side_effect_free` | `check-session-history-checkpoint-replay-contract.*` | `session_history_checkpoint_replay.v1` |
...
| `adapter-onboarding-manifest-capability` | `P2` | `adapter.manifest_capability_fallback` | `governance_adapter_gate_enforced; governance_adapter_replay_bound` | `check-adapter-manifest-contract.*; check-adapter-capability-contract.*; check-adapter-contract-replay.*; check-provider-model-capability-contract.*` | `adapter_contract_profile.v1; provider_model_admission.v1` |
