...
func DefaultConfig() Config {
	return Config{
		MCP: MCPConfig{
			ActiveProfile: string(ProfileDefault),
			Profiles: map[string]types.MCPRuntimePolicy{
				string(ProfileDev):            defaultPolicyFor(ProfileDev),
				string(ProfileDefault):        defaultPolicyFor(ProfileDefault),
				string(ProfileHighThroughput): defaultPolicyFor(ProfileHighThroughput),
				string(ProfileHighReliab):     defaultPolicyFor(ProfileHighReliab),
			},
		},
...
func LoadConfig(opts LoadOptions) (Config, error) {
	cfg, _, err := loadConfigWithSnapshot(opts)
	return cfg, err
}

func loadConfigWithSnapshot(opts LoadOptions) (Config, map[string]any, error) {
	v := viper.New()
...
func ResolvePolicyWithConfig(cfg Config, profile ProfileName, override *types.MCPRuntimePolicy) (types.MCPRuntimePolicy, error) {
